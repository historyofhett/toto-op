  ...
  run: async (client, message) => {
    ...
    if (message.content.match(mention)) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({
          text: `Love From ForceX's Team <3`,
      ...
      const kabbu = await client.users.fetch(`937253889252675696`);
    ...
    if (command.owner && message.author.id !== `${client.owner}`) {
      const ozuma = await client.users.fetch(`803285289358983199`);
      const Ozuma_xd = await client.channels.cache.get(`937253889252675696`);
